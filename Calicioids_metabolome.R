...
beeswarm(richness$Bacteria ~ richness$Treatment, 
         ...
         pwcol = my.pwcol1, cex.axis = 0.7, 
         cex.main = 0.8, labels = my.labels1)
title(xlab = "Sample Type", mgp = c(1.75, 3, 0)) 
...
Type <- c("Culture","Culture","Culture",
...
ggplot(plot_data, aes(x = x, y = y)) +
  ...
  geom_point(aes(colour = Type, shape = Substrate),
             alpha = 1, size = 3) +
  ...
  labs(x = "Metabolome Diversity (H')", 
       ...
       subtitle = paste("R-squared = 0.275, ", 
                        "p-value = 0.002")) +
  theme_minimal()
#dev.off()
...
##################### PROCRUSTES #######################
result <- procrustes(CSCS, UF_ITS, scale = TRUE)
summary(result)
plot(result)


# Run Procrustes analysis
proc_result <- procrustes(dbrda_MET, dbrda_ITS)
